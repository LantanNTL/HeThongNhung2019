...
import read_data_from_stm as live_plot
...
# hàm insert dữ liệu nhiệt độ độ ẩm vào database
...
def choose_port_setup():
	...
	print(Ports)
	lbl_select = Label(app,text = "Please select COM port!",bg = "white",
	...
	lbl_select = Label(app,fg= "black",font=("Times",16,'bold'));
	lbl_select.pack();
	btn_select = Button(app,text = "Choose",bg = "white",fg= "green",
		font=("Times",16,'bold'),command= start)
	btn_select.pack();
...
def setup_ui():
	...
	lbl_temp = Label(ui,text = "Temperature(°C): ",bg="white",fg = "black",font=("Times",16,'bold'));
